...
class Application
{
	...
	var $conf=[
		...
		'roles'=> [
			'tablename'=>'roles',
			#'baslik'=>['id','rolename','ilgili proje','ilgili kullanıcı'),
...
function check_php_version(){
	...
	if(version_compare(phpversion(), '7.0.0', '<')) {  # make compatible install in many versions of Ubuntu/Debian, Ubuntu 16.04 comes with php7, while olders are different
		...
		$this->php_sock="unix:/run/php/php{$ver}-fpm.sock"; # fastcgi_pass    unix:/run/php/php7.2-fpm.sock;   # buna gerek kalmadı, zira php_etc_dir doğru hesaplayacak ve listen = 9000 yapacak. nginx configler de ona göre ayarlı. 
	}
...
function ssl_certbot($domainname){
	...
	*/
	...
	/* 
	Bundan sonra, başarılı ise, ssl configin yüklenmesi ve keylerin yerine konulması yapılacak.. 
	/etc/letsencrypt/live/xxx.com altında oluyor.. 
	
	burada, domain bazında ssle geçme yapmam lazım. 
	domain tablosuna alan ekleyip, ssl enable olan domainleri webserver_template_ssl den yüklemem lazım. 
	Ayrıca, php-fpm in hangi porttan dinlediğini bulmak lazım. 9000 veya unix socket:
...
Bu ayara şimdilik dokunmadım, install_lib içinde var. 9000 olarak kaldı. 
...
function executeProg3($prog,$echooutput=False,$debug=False){
	...
	if($debug) $prog.=" 2>&1 "; # ozellikle, programların hatalarını bulmak için faydalı olabilir.
...
*/
	...
	$s=$this->executeQuery($q,'domain add to ehcp db'); # multiserver ayri bir fonksiyona yazıldı.
...
function query2($qu) { # sadece tek satır döndürür
	$res=$this->query($qu);
...
function mylink() {
	if(!$this->conn or !$this->connected) {
		...
		return $this->conn->_connectionID; # bunu adodb kodlarından öğrendim.
	}
}
...
function get_dnsservers(){
	...
	if(count($localserver)==0) $dnsservers[]=['servertype'=>'binddns','ip'=>'localhost']; # add localhost if not exist # binddns harici bir sey kullanılacaksa, modifiye edilmeli.get_webservers a bak.
...
function initializePhpTmpDir($subdir){
	...
	passthru4("chown $this->ftpowner -Rf $subdir/phptmpdir"); # **** Buradaki problem şu: phptmpdir içinde yeni oluşturulan dosyalar -rw------- 1 www-data www-data şeklinde oluşturuluyor. burası da sahipliğini vsftpd yapınca, artık apache bunu silemez oluyor. burayı -rw-rw--- şeklinde yapmak lazım. Problem: http://ehcp.net/?q=node/1351#comment-2831 ; Bunu umask ile çözdüm sanırım.
