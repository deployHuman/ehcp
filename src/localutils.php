...
function writeoutput2($file, $string, $mode="w",$debug=false) {
	$file=removeDoubleSlash($file);
	echo "\n".__FUNCTION__.":*** Writing to file ($file) \n";
...
function writeoutput3($file, $string, $mode="w",$debug=false) {
	$file=removeDoubleSlash($file);
...
function getlocalip($interface='eth0') {
	global $localip;
	// print "Trying to find ip of interface-localutils: $interface\n";
...
function passthru4($cmd,$no_remove=false,$no_escape=false){ # echo yapmaz
	return passthru2($cmd, $no_remove, $no_escape, False);
}

function passthru2($cmd,$no_remove=false,$no_escape=false, $echo=True){
	$cmd1=$cmd;
	if(!$no_remove) $cmd=removeDoubleSlash($cmd);
	if(!$no_escape) $cmd=escapeshellcmd($cmd);
	if($echo) echo "executing command3: $cmd1 \n";
	passthru($cmd);
	return true;
}
