...
# last modified by ehcpdeveloper on 6.4.2020 (d-m-y)
...
Will be updated according to Ubuntu 20.04
...
if(version_compare(phpversion(), '7.0.0', '<')) {  # make compatible install in many versions of Ubuntu/Debian, Ubuntu 16.04 comes with php7, while olders are different
	# similar code exist in classapp.php, sync both code !
	$php_version=5;
	$php_version_tag="5";
	...
	$php_version=7;
	$php_version_tag="";	
	$php_etc_dir="/etc/php/".$ver;
	$php_fpm="php{$ver}-fpm";
	$php_sock="unix:/run/php/php{$ver}-fpm.sock"; # fastcgi_pass    unix:/run/php/php7.2-fpm.sock;   # buna gerek kalmadı, zira php_etc_dir doğru hesaplayacak ve listen = 9000 yapacak. nginx configler de ona göre ayarlı. 
}
...
function replace_or_add($find, $replace, $where) {
	replacelineinfile($find, $replace,$where);
	addifnotexists($replace,$where);
}
...
function getlocalip($interface='eth0') {
	global $localip;
	// print "Trying to find ip of interface: $interface\n";
...
passthru3("postconf -e \"mydestination = localhost, $ip, [::1]/128 \"");
...
echo "Now configuring to tell Courier that it should authenticate against our MySQL Ehcp database.";
...
So, imap auth used for authentication. 
*/

replace_or_add('MECHANISMS="', 'MECHANISMS="rimap"',"/etc/default/saslauthd");
replace_or_add('MECH_OPTIONS="', 'MECH_OPTIONS="127.0.0.1"',"/etc/default/saslauthd");
...
# end adjust roundcube
	...
	# https://ehcp.net/?q=node/1920
	$cmds=[
		"rm -Rvf /var/run/saslauthd",
		"ln -s /var/spool/postfix/var/run/saslauthd /var/run/saslauthd",
		"chown root:sasl /var/spool/postfix/var/run/saslauthd",
		"usermod -a -G sasl postfix",
	];
	runcmds($cmds);
	// if (! file_exists("/var/run/saslauthd")) { 
	// 	passthru("ln -s /var/spool/postfix/var/run/saslauthd /var/run/saslauthd"); 
	// }
...
"; # marker line may be needed in Ubuntu 17.10 and up

writeoutput("/etc/courier/authmysqlrc",$filecontent,"w");

replacelineinfile("MAXDAEMONS=","MAXDAEMONS=10","/etc/courier/imapd");
replacelineinfile("MAXPERIP=","MAXPERIP=10","/etc/courier/imapd");
...
function check_restart_mysql($checkpass=False){
	...
	while (!$ret) {
		...
		if($try<=3) {
			...
			echo "Unfortunately, mysql has still not started after 10 tries. Check the problem on a separate command line.. sorry.\n\n";
			getInput();
			return;
		}
		sleep(2);
...
function installRoundCube(){#by earnolmartin@gmail.com
	...
	aptget(['roundcube', 'roundcube-mysql','roundcube-plugins','roundcube-plugins-extra', 'roundcube-plugins', 'roundcube-core'],$usePrompts);
}
...
function installmailserver(){
	...
	# Avoid error: update-alternatives: error: alternative path /usr/share/man/man5/maildir.courier.5.gz doesn't exist
	# https://askubuntu.com/questions/1054233/courier-base-fails-to-install-on-ubuntu-16-04-4-lts-base-install

	$cmds=["touch /usr/share/man/man5/maildir.courier.5.gz",
	...
	switch($installmode) {
		...
		case 'light':
			...
			aptget(['courier-authdaemon','courier-authlib-mysql','courier-pop','courier-pop-ssl','courier-imap','courier-imap-ssl','libsasl2-2','libsasl2','libsasl2-modules','libsasl2-modules-sql','sasl2-bin','libpam-mysql','openssl','pop-before-smtp'],False,True); # changed libsasl2-2 to libsasl2 **
			break;
		default: echo "Unknown installmode parameter at ".__LINE__;
	}

	passthru("apt-get install libgamin0 -y"); # Fix for "Filesystem notification initialization error" while sendine email via smtp
	...
	bosluk2();
	# all mail configuration should be moved into this function, to be able to re-configure mail later
	mailconfiguration(['ehcppass'=>$ehcpmysqlpass]);
	mailNameFix();

	echo "\n\nfinished mail server,pop3,imap installation \n";
}

function read_replace_write($src, $dst, $find, $repl) {
	$conf=file_get_contents($src);
	$conf=str_replace($find,$repl,$conf);
	file_put_contents($dst,$conf);
}

function rebuild_nginx_config2($mydir){
	global $app,$php_etc_dir, $php_version_tag, $php_service_name;

	print(__FUNCTION__.": php version:".phpversion()."  php_version_tag:".$php_version_tag."  php_service_name: $php_service_name \n");

	passthru3("rm -rvf /etc/nginx/sites-enabled/*");

	#passthru2("cp $mydir/etc/nginx/nginx.conf /etc/nginx/nginx.conf");
	read_replace_write("$mydir/etc/nginx/nginx.conf", "/etc/nginx/nginx.conf", ['{wwwuser}','{wwwgroup}'],[$app->wwwuser,$app->wwwgroup]);

	// $conf=file_get_contents("$mydir/etc/nginx/nginx.conf"); # replace tags with actual values from class
	// $conf=str_replace(['{wwwuser}','{wwwgroup}'],[$app->wwwuser,$app->wwwgroup],$conf);
	// file_put_contents("/etc/nginx/nginx.conf",$conf);


	# Normally, should be: copy only templates to templates folder. Actual config will be built in configs folder by classapp
	passthru2("cp -vf $mydir/etc/nginx/default /etc/nginx/sites-enabled/");
	passthru2("cp -vf $mydir/etc/nginx/default $mydir/etc/current_webserver_templates/");
	passthru2("cp -vf $mydir/etc/nginx/webserver_template $mydir/etc/current_webserver_configs/");
	passthru2("cp -vf $mydir/etc/nginx/webserver_template $mydir/etc/current_webserver_templates/");
	passthru2("cp -vf $mydir/etc/nginx/webserver_subdomain_template $mydir/etc/current_webserver_configs/");	
	passthru2("cp -vf $mydir/etc/nginx/webserver_subdomain_template $mydir/etc/current_webserver_templates/");
	passthru2("cp -vf $mydir/etc/nginx/webserver_subdomain_template_ipbased $mydir/etc/current_webserver_configs/");	
	passthru2("cp -vf $mydir/etc/nginx/webserver_subdomain_template_ipbased $mydir/etc/current_webserver_templates/");

	// replacelineinfile("listen =","listen = 9000","$php_etc_dir/fpm/pool.d/www.conf"); # I leave as default. 
	// replacelineinfile("listen =","listen = /run/php/php7.0-fpm.sock","$php_etc_dir/fpm/pool.d/www.conf");
	
	/*
	This is done by detecting php version and using listen 9000 in related php conf above, in 
	To be done:  (similar affect Already done above, however, if fpm version is incorrect, it does not work.. )
	php-fpm port listening type will be detected (sock or port 9000)
	config files will be fixed accordingly..
	...
	In Debian:
	php-fpm version is not detected correctly... in ver 0.38.3.b
	*/
	
	passthru2("/usr/sbin/service $php_service_name restart"); # this does not work on some systems.. needs another binary to work
}

function install_nginx_webserver(){
	global $php_version_tag;

	# thanks to webmaster@securitywonks.net for encourage of nginx integration
	passthru2("/usr/sbin/service apache2 stop"); # aksi takdirde, nginx kurulumu tamamlanmıyor.
	echo "\nStarting nginx webserver install (not default)\n";
	#bekle();
	aptget(['nginx',"php{$php_version_tag}-fpm","php{$php_version_tag}-cgi"]);  # apt-get install nginx php5-fpm php5-cgi
	...
	passthru2("/usr/sbin/service php{$php_version_tag}-fpm stop");
	passthru2("/usr/sbin/update-rc.d -f nginx remove");  # apache is default
	passthru2("/usr/sbin/service nginx stop");
...
function installapacheserver($apacheconf=''){
	global $app,$ehcpinstalldir, $php_version, $php_version_tag, $php_etc_dir;
	echo "\nStarting apache2 webserver install (default webserver) Detected approx php version: $php_version \n";
	#bekle(__FUNCTION__." basliyor..");

	aptget(["libapache2-mod-php{$php_version_tag}","php{$php_version_tag}"]);
	rebuild_apache2_config2();
}

function rebuild_apache2_config2(){
	global $app,$ehcpinstalldir, $php_version, $php_version_tag, $php_etc_dir, $copy_counter;
	echo "\n---------------------------\n";
	echo __FUNCTION__;
    echo "* installpath: $ehcpinstalldir, php_version:$php_version, php_etc_dir:$php_etc_dir\n";

	$tarih=date("YmdHis");
	if($copy_counter<200) { # Avoid thousands of backups
		$copy_counter++;
		passthru2("cp -rf /etc/apache2 /etc/apache2.backupbyehcp.$tarih"); 
	}

	sleep(3); # bc: in loop, lots of such dirs.
	addifnotexists("Include $ehcpinstalldir/etc/current_webserver_configs/webserver_ehcp_subdomains.conf ", "/etc/apache2/apache2.conf");
	addifnotexists("Include $ehcpinstalldir/etc/current_webserver_configs/webserver_ehcp_auth.conf ", "/etc/apache2/apache2.conf");
	addifnotexists("Include $ehcpinstalldir/etc/current_webserver_configs/webserver_ehcp_passivedomains.conf ", "/etc/apache2/apache2.conf");
	addifnotexists("Include $ehcpinstalldir/etc/current_webserver_configs/webserver_ehcp.conf", "/etc/apache2/apache2.conf");
	
	replacelineinfile("IncludeOptional sites-enabled/*", "Include sites-enabled/*", "/etc/apache2/apache2.conf"); # Avoid duplicate
	addifnotexists("Include sites-enabled/*", "/etc/apache2/apache2.conf");

	# disable old format lines, in old ehcp installs.
	replacelineinfile("Include /var/www/new/ehcp/apachehcp_subdomains.conf","# Include /var/www/new/ehcp/apachehcp_subdomains.conf","/etc/apache2/apache2.conf");
	replacelineinfile("Include /var/www/new/ehcp/apachehcp_auth.conf","# Include /var/www/new/ehcp/apachehcp_auth.conf","/etc/apache2/apache2.conf");
	replacelineinfile("Include /var/www/new/ehcp/apachehcp_passivedomains.conf","# Include /var/www/new/ehcp/apachehcp_passivedomains.conf","/etc/apache2/apache2.conf");
	replacelineinfile("Include /var/www/new/ehcp/apachehcp_","# Include /var/www/new/ehcp/apachehcp_","/etc/apache2/apache2.conf");
	...
	if(file_exists("/etc/apache2/envvars")) {
		...
		addifnotexists("umask 0000","/etc/apache2/envvars"); # newly added files wil have rw-rw-rw- permission, allowing all to read-write them. this will enable vsftpd to delete webserver-generated files.. before this, those files could not be deleted...
	} else {
	...
	passthru2("cp -vf $ehcpinstalldir/etc/apache2/webserver_template_ipbased $ehcpinstalldir/");
	passthru2("cp -vf $ehcpinstalldir/etc/apache2/webserver_template $ehcpinstalldir/");
	passthru2("cp -vf $ehcpinstalldir/etc/apache2/webserver_template_passivedomains $ehcpinstalldir/");
	passthru2("cp -vf $ehcpinstalldir/etc/apache2/webserver_subdomain_template $ehcpinstalldir/");
	...
	passthru2("/usr/sbin/a2enmod php{$php_version_tag}");
	passthru2("/usr/sbin/a2enmod expires");
	passthru2("/usr/sbin/a2enmod headers");
	passthru2("cp -vf /etc/apache2/mods-available/php{$php_version}.* /etc/apache2/mods-enabled/");
	...
	addifnotexists("extension=mysql.so","$php_etc_dir/apache2/php.ini");
	addifnotexists("extension=mysql.so","$php_etc_dir/cli/php.ini");
...
# allow_writeable_chroot=YES : bunun uzerinde calisacagim... boyle olmuyor... bakalim... bu olabilir: http://ehcp.net/?q=comment/2905#comment-2905
	...
	passthru("/usr/sbin/service vsftpd restart");


}

function install_pyftp_server() {
	global $app;

	passthru("useradd -m -g {$app->ftpgroup} -u 5001 {$app->ftpuser} -s /usr/sbin/nologin");
	aptget(["python-pip", "python3-pip", "python3-mysqldb"]);
	passthru("pip3 install pyftpdlib");
	touch("/var/log/ehcp_ftpd.log");
	chmod("/var/log/ehcp_ftpd.log", 0777);
	# ftp_server2.py file is already in ehcp dir. it reads users from db 
}
...
function install_vsftpd_server(){
	...
	aptget(['vsftpd','python-mysqldb','python-pip']); # last 2 is for alternate python ftp server.
	passthru("pip install pyftpdlib");
	passthru("pip3 install pyftpdlib");
	...
	vsftpd_configuration(['ehcppass'=>$ehcpmysqlpass]);
}
...
function installsql() {
	...
	if($tmprootpass!="") { # try with empty password, for unattended etc..
...
function getinputs(){
	...
	writeoutput('/etc/ehcp/ehcp.conf',"[ehcp]\nehcpdir=$ehcpinstalldir","w");
	...
	$lang=getInput("enter language file you want to use (en/tr/german/spanish/nl/fr/lv/pt_br	 [default en]):",'en');
...
function initialize() {
	global $ehcpinstalldir,$app,$ip,$user_email,$user_name,$yesno,$ehcpmysqlpass,$rootpass,$newrootpass,$ehcpadminpass,$installmode,$php_etc_dir, $php_version_tag;
	#echo "Starting ".__FILE__.":".__FUNCTION__."\n";

	infomailusingwget("3_ehcpinstall.php-started");
	passthru("/usr/sbin/service apparmor teardown");
	...
	editlineinfile(['#deb','##deb','# deb'],['deb','deb','deb'],'/etc/apt/sources.list');
...
// Also note that, a reseller account of 'ehcp' is setup by default to support ehcp, only in free version. you may delete it after install. (Please don't)
# ehcp account removed as of 0.38.6, we don't need it as of now, thank you for your supports up to now.
	...
	switch($installmode) {
		case 'extra': aptget(['mc','lynx','nmap']);
		case 'normal': aptget(['unrar','rar','unzip','zip']);
		case 'light': aptget(['openssh-server','python-mysqldb','python-cherrypy3','apache2','bind9',"php{$php_version_tag}-gd",'libapache-mod-ssl','curl']);
			break;
		default: echo "Unknown installmode parameter at ".__LINE__;
	}
...
function install_net2ftp(){
	...
	passthru2("unzip -q net2ftp_v1.0.zip");
	passthru2("mv -f net2ftp_v1.0/files_to_upload $ehcpinstalldir/net2ftp");

	net2ftp_configuration(['ehcppass'=>$ehcpmysqlpass,'ehcpinstalldir'=>$ehcpinstalldir]);
...
function installfinish() {
	global $ehcpinstalldir,$ehcpmysqlpass,$app,$user_email,$user_name,$header,$installextrasoftware,$lightinstall,$installmode,$emailenable,$php_etc_dir;

	switch($installmode) {
		# ffmpeg not supported directly for php7 yet.. 
		case 'extra': aptget(['postgrey','ffmpeg','php5-ffmpeg','mplayer','mencoder','nmap','listadmin','spamassassin','aptitude','gpac','clamav','clamav-freshclam','libavcodec-unstripped','byobu']);
		case 'normal': aptget(['webalizer','php-pear','phpsysinfo','mailutils', 'byobu', 'certbot']);
		case 'light': aptget(["bind9-host","php{$phpversion_tag}-curl","php{$phpversion_tag}-xmlrpc","php{$phpversion_tag}-imap", 'ftp', 'rsyslog']);
			break;
		default: echo "Unknown installmode parameter at ".__LINE__;
	}
	...
	# fix syslog, if broken:
	touch('/var/log/syslog');
	chown('/var/log/syslog', 'syslog.adm');
	# fix postfix isue, if any:  https://ubuntuforums.org/showthread.php?t=2322615
	passthru2('ln -s /usr/lib/postfix/postfix-mysql.so.1.0.1 /usr/lib/postfix/dict_mysql.so');
	...
	# $app->syncdomains($app->ehcpdir."/webserver_ehcp.conf");

	passthru("/usr/sbin/service sendmail stop");
	...
	passthru("php{$phpversion_tag}enmod mcrypt");
	passthru("mkdir -p $php_etc_dir/conf.d"); # if not exists
	passthru("ln -s $php_etc_dir/conf.d/mcrypt.ini $php_etc_dir/mods-available"); # fix if missing; http://ehcp.net/?q=comment/3072#comment-3072

	echo "\nPlease wait while services restarting...\n\n";
	passthru("/usr/sbin/service nginx stop");
	passthru("/usr/sbin/service apache2 restart");
	passthru("/usr/sbin/service bind9 restart");
	passthru("/usr/sbin/service named restart"); # Ubuntu 20.04
	passthru("/usr/sbin/service postfix restart");
	...
	sleep(2);
	check_restart_mysql();
	passthru2("/usr/sbin/update-rc.d -f nginx remove");
	passthru2("/usr/sbin/update-rc.d apache2 defaults");
	passthru("/usr/sbin/service apparmor teardown");
	passthru("/etc/init.d/courier-authdaemon restart"); # daemons=1
...
function scandb(){
	...
	passthru("/usr/sbin/service apache2 reload");
...
function infomail($str,$msg=''){
	...
	$ip=getlocalip2();
	if($msg=='') $msg=$str."\nName: $user_name \nEmail:$user_email";
	mail('info@ehcp.net',"ehcp-install-$ehcpversion-$ip-".$str,$msg,$header);
}
