...
# althaugh this started as a basic installer, now, not very basic... :)
#
# please contact us if you made any modifications.. or you need help
# email: info@ehcp.net, support@ehcp.net
# skype:ehcpanel
#
# Main Developer: info@ehcp.net
# Others who contributed in some amount:
...
# Paypal donation: look to ehcp.net website
# Note also that, ehcp has also a Professional version. check ehcp.net website for details. 
...
function check_programs(){
	...
	do
		php -l $i > /dev/null
		if [ $? -ne 0 ] ; then
			echo "Important/Syntax Error in Program: $i"
			echo "You may continue, but you may live some problems (Press Enter to continue)"
			read a
		fi
	done
...
# Todo: in Unattended mode, passwords can be read from a file, to which can be written manually or by any automation program, use case: docker etc. 
...
function ehcpHeader() {
	...
	echo "If any problem occurs, refer to www.ehcp.net forum section, or contact us, mail: info@ehcp.net"
	echo "Ehcp is specifically tested for Ubuntu, but should also work on many Debian based distros."
	echo ""
	...
	echo "Ehcp also sends some usage data (including running services) to developer for statistical/improvement purposes"
	echo
...
function ubuntuVSFTPDFix(){ # by earnolmartin@gmail.com
	...
	# this is not used starting with Ubuntu 16.04. Switched to a python based ftp server. if anyone nows some other good ftp server with mysql support, let us know.
	origDir=$( pwd )
...
function check_mysql(){
	...
	say=`php test_mysql.php | grep "t connect to local MySQL server through socket" | wc -l `
	if [ $say -gt 0 ] ; then
		echo "I am sorry that, mysql seems still not working. We/You should fix this... This may be serious. Your panel may not work without mysql."
	fi
}
...
ehcpHeader
aptget_Update
aptgetInstall dnsutils net-tools iputils-ping iproute2 # otherwise, host, ifconfig command may be missing, that is used in following line. 
check_internet
aptgetInstall apt-utils psmisc
...
aptgetInstall net-tools # otherwise, ifconfig not exists on some systems
aptgetInstall python-software-properties # for add-apt-repository ; prior to U18.04
aptgetInstall software-properties-common # for add-apt-repository ; Ubuntu 18.04 fix
...
aptgetInstall toilet
toilet -F gay EHCP

# These are no longer default supported in newer Ubuntu's (after 16.04)
# aptgetInstall php5
...
# Thanks to Nilesh Anand for suggestions; https://www.youtube.com/watch?v=Zf9cfS7a6k4
aptgetInstall build-essential
aptgetInstall dpkg-dev
...
Note that, ehcp supports to install many web based Apps (Wordpress, Drupal etc.) automatically through its interface, through so called 'Easy Install Scripts'. You may also suggest other scripts through same interface.

ehcp install finished up to now.  we are continuing on simplifying the install process.
sorry for any inconvenience. you can contact email: info@ehcp.net

You may:
- Donate/support ehcp : http://ehcp.net/?q=node/25
- consider to buy a pro license
- join us in developing this control panel.
- visit http://www.ehcp.net for more info
- support by donating cash, buying a pro license, doing php/python coding, html design, graphic design... http://ehcp.net/?q=node/25
- support by donating free dedicated or virtual servers for this project...

CURRENTLY WE NEED A DEDICATED SERVER WITH 8 CORE, 16GRAM, 500G hdd at least
